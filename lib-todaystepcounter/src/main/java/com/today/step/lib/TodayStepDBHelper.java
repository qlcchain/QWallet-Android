...
class TodayStepDBHelper extends SQLiteOpenHelper implements ITodayStepDBHelper {
    ...
    public synchronized boolean isExist(TodayStepData todayStepData) {
        Cursor cursor = getReadableDatabase().rawQuery(SQL_QUERY_STEP, new String[]{todayStepData.getToday()});
     ...
     */
    @Override
    public synchronized void clear(String curDate) {
        try {
            getWritableDatabase().execSQL(SQL_DELETE_TODAY, new String[]{curDate});
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
