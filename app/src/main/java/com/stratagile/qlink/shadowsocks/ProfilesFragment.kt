...
import com.stratagile.qlink.shadowsocks.plugin.PluginConfiguration
...
import com.stratagile.qlink.shadowsocks.widget.UndoSnackbarManager
...
class ProfilesFragment : ToolbarFragment(), Toolbar.OnMenuItemClickListener {
     ...
     */
    private val isEnabled get() = when ((activity as ShadowVpnActivity).state) {
        BaseService.CONNECTED, BaseService.STOPPED -> true
        else -> false
    }
    private fun isProfileEditable(id: Long) = when ((activity as ShadowVpnActivity).state) {
...
//            }
        ...
        fun bind(item: Profile) {
            ...
            text2.text = ArrayList<String>().apply {
                ...
                if (id.isNotEmpty()) this += AppConfig.instance.getString(R.string.profile_plugin, id)
            }.joinToString("\n")
        ...
        override fun onClick(v: View?) {
            if (isEnabled) {
                val activity = activity as ShadowVpnActivity
                val old = DataStore.profileId
                AppConfig.instance.switchProfile(item.id)
                profilesAdapter.refreshId(old)
                itemView.isSelected = true
                if (activity.state == BaseService.CONNECTED) AppConfig.instance.reloadService()
            }
        }
...
//                profilesAdapter::undo, profilesAdapter::commit)
        ...
        ItemTouchHelper.START or ItemTouchHelper.END) {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                profilesAdapter.remove(index, (viewHolder as ProfileViewHolder).item.id)
...
//                startActivity(Intent(context, ScannerActivity::class.java))
            ...
            R.id.action_import_clipboard -> {
                try {
                    val profiles = Profile.findAllUrls(clipboard.primaryClip!!.getItemAt(0).text, AppConfig.instance.currentProfile).toList()
            ...
            R.id.action_manual_settings -> {
                startConfig(ProfileManager.createProfile(
                        Profile().also { AppConfig.instance.currentProfile?.copyFeatureSettingsTo(it) }))
                true
            }
            R.id.action_export_clipboard -> {
                val profiles = ProfileManager.getAllProfiles()
                toast(if (profiles != null) {
...
//                (activity as ShadowVpnActivity).snackbar().setText(if (profiles != null) {
//                    clipboard.primaryClip = ClipData.newPlainText(null, profiles.joinToString("\n"))
//                    R.string.action_export_msg
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (resultCode != Activity.RESULT_OK) super.onActivityResult(requestCode, resultCode, data)
            ...
            REQUEST_IMPORT_PROFILES -> {
                ...
                toast(if (success) R.string.action_import_msg else R.string.action_import_err)
...
//                        .show()
            ...
            REQUEST_EXPORT_PROFILES -> {
                ...
                if (profiles != null) try {
                    ...
                    printLog(e)
                    ToastUtil.displayShortToast(e.localizedMessage)
                }
            }
