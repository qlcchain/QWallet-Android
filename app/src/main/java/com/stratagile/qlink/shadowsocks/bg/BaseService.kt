...
package com.stratagile.qlink.shadowsocks.bg
...
import com.stratagile.qlink.shadowsocks.utils.*
                    ...
                    if (timer == null) {
                        ...
                        t.schedule(object : TimerTask() {
                            override fun run() {
                                ...
                                if (state == CONNECTED && TrafficMonitor.updateRate()) AppConfig.instance.handler.post {
...
//                        .put("plugin_opts", plugin.toString())
            ...
            return File(if (UserManagerCompat.isUserUnlocked(AppConfig.instance)) AppConfig.instance.filesDir else @TargetApi(24) {
                AppConfig.instance.deviceStorage.noBackupFilesDir  // only API 24+ will be in locked state
            }, CONFIG_FILE).apply {
        ...
        fun changeState(s: Int, msg: String? = null) {
            ...
            if (callbacks.registeredCallbackCount > 0) AppConfig.instance.handler.post {
                val n = callbacks.beginBroadcast()
                for (i in 0 until n) try {
    ...
    interface Interface {
        ...
        fun forceLoad() {
            val profile = AppConfig.instance.currentProfile
...
//            AppConfig.instance.analytics.logEvent("stop", bundleOf(Pair(FirebaseAnalytics.Param.METHOD, tag)))
...
//            AppConfig.instance.analytics.logEvent("start", bundleOf(Pair(FirebaseAnalytics.Param.METHOD, tag)))
...
//                        mdg.update(AppConfig.instance.info.signaturesCompat.first().toByteArray())
//                        val requestBody = FormBody.Builder()
//                                .add("sig", String(Base64.encode(mdg.digest(), 0)))
//                                .build()
//                        val request = Request.Builder()
//                                .url(AppConfig.instance.remoteConfig.getString("proxy_url"))
//                                .post(requestBody)
//                                .build()
//
