...
object PluginManager {
    ...
    val trustedSignatures by lazy {
        AppConfig.instance.info.signaturesCompat.toSet() +
    ...
    fun fetchPlugins(): Map<String, Plugin> = synchronized(this) {
        if (receiver == null) receiver = AppConfig.instance.listenForPackageChanges(true) {
            synchronized(this) {
                receiver = null
                cachedPlugins = null
            }
        ...
        if (cachedPlugins == null) {
            val pm = AppConfig.instance.packageManager
    ...
    fun init(options: PluginOptions): String? {
        ...
        throw throwable
                ?: FileNotFoundException(AppConfig.instance.getString(R.string.plugin_unknown, options.id))
    }

    private fun initNative(options: PluginOptions): String? {
        val providers = AppConfig.instance.packageManager.queryIntentContentProviders(
        ...
        val uri = Uri.Builder()
                ...
                .build()
        val cr = AppConfig.instance.contentResolver
        return try {
...
//                    "Initializing native plugin fast mode failed. Falling back to slow mode.")
    ...
    private fun initNativeSlow(cr: ContentResolver, options: PluginOptions, uri: Uri): String? {
        ...
        val pluginDir = File(AppConfig.instance.deviceStorage.filesDir, "plugin")
