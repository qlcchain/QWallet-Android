...
import com.stratagile.qlink.shadowsocks.utils.Subnet
import com.stratagile.qlink.shadowsocks.utils.getSystemService
import com.stratagile.qlink.shadowsocks.utils.parseNumericAddress
...
class VpnService : BaseVpnService(), LocalDnsService.Interface {
    ...
    private inner class ProtectWorker : LocalSocketListener("ShadowsocksVpnThread") {
        override val socketFile: File = File(AppConfig.instance.deviceStorage.filesDir, "protect_path")
...
//                    .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
    ...
    private fun startVpn(): Int {
        ...
        val builder = Builder()
                .setConfigureIntent(ShadowVpnActivity.pendingIntent(this))
    ...
    private fun sendFd(fd: Int): Boolean {
        if (fd != -1) {
            ...
            while (tries < 10) {
                ...
                if (JniHelper.sendFd(fd, File(AppConfig.instance.deviceStorage.filesDir, "sock_path").absolutePath) != -1) return true
                tries += 1
            }
        }
        return false
