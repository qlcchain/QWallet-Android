...
class TopupAbleAdapter(array: ArrayList<TopupProduct.ProductListBean>) : BaseQuickAdapter<TopupProduct.ProductListBean, BaseViewHolder>(R.layout.item_topup_able, array) {
    ...
    override fun convert(helper: BaseViewHolder, item: TopupProduct.ProductListBean) {
        ...
        helper.setText(R.id.qgasCount, item.price.toBigDecimal().multiply(item.qgasDiscount.toBigDecimal()).divide(payToken.price.toBigDecimal(), 3, BigDecimal.ROUND_HALF_UP).stripTrailingZeros().toPlainString())
        helper.setText(R.id.payTokenSymbol, payToken.symbol)
