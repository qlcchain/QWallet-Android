package com.stratagile.qlink.walletconnect
...
import com.stratagile.qlink.hexStringToByteArray1
import com.stratagile.qlink.walletconnect.entity.*
import com.stratagile.qlink.walletconnect.util.WCCipher
import okhttp3.*
...
open class WCClient(
    ...
    private fun decryptMessage(text: String): String {
        ...
        KLog.i(session.topic)
        KLog.i(session.key)
        return String(WCCipher.decrypt(encrypted, session.key.hexStringToByteArray1()), Charsets.UTF_8)
    }
    ...
    private fun encryptAndSend(result: String): Boolean {
        ...
        val session = this.session
                ?: throw IllegalStateException("Session is null")
        val payload = gson.toJson(WCCipher.encrypt(result.toByteArray(Charsets.UTF_8), session.key.hexStringToByteArray1()))
        val message = WCSocketMessage(
