...
import static com.stratagile.qlink.core.OpenVPNManagement.pauseReason;
...
public class DeviceStateReceiver extends BroadcastReceiver implements ByteCountListener, OpenVPNManagement.PausedStateCallback {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        } else if ("com.stratagile.qlink.disconnectVpn".equals(intent.getAction())) {
            KLog.i("断开连接的广播。。。。");
            mManagement.stopVPN(false);

        } else if ("com.stratagile.qlink.VPN_STATUS".equals(intent.getAction())) {
    ...
    public void networkStateChange(Context context) {
        ...
        if (networkInfo != null && networkInfo.getState() == State.CONNECTED) {
            ...
            // Not connected, stop qlink, set last connected network to no network
            lastNetwork = -1;
            if (sendusr1) {
