...
import com.stratagile.qlink.utils.MIUIUtils;
...
public class Utils {
    ...
    public static Intent getFilePickerIntent(Context c, FileType fileType) {
        ...
        switch (fileType) {
            ...
            case OVPN_CONFIG:
                i.setType("application/x-winq-profile");
                supportedMimeTypes.add("application/x-winq-profile");
                supportedMimeTypes.add("application/winq-profile");
...
//                supportedMimeTypes.add("text/plain");
        ...
        KLog.i(Build.MANUFACTURER);
        LogUtil.addLog("厂商为：" + Build.MANUFACTURER, "");
        LogUtil.addLog("是否为miui：" + MIUIUtils.isMIUI(), "");
        if (!MIUIUtils.isMIUI()) {
